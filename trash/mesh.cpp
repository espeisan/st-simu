...
//      if (nod_id || nod_is){
//          for (int l = 0; l < LZ; l++){
//            node_dofs_solid_fs(l) = n_unknowns_u + n_unknowns_p + LZ*(nod_id+nod_is) - LZ + l;
//          }
//      }
//      else{
...
#if (false)
          if (nod_id){
            VecGetValues(Vec_up_0,  LZ, node_dofs_solid_fs.data(), U0_fs.data());
            VecGetValues(Vec_up_1,  LZ, node_dofs_solid_fs.data(), U1_fs.data());
            if(!SV[nod_id-1]){
              if (time_step == 0){
                Xgt = XG[nod_id-1];
                Xg(0) = Xgt(0); Xg(1) = Xgt(1); if (dim == 3){Xg(2) = Xgt(2);}//getAreaMassCenterSolid(nod_id, Ar); AV[nod_id-1] = Ar;
              }
              else{
                Vels(0) = U1_fs(0);  Vels(1) = U1_fs(1); if (dim == 3){Vels(2) = U1_fs(2);}
                Xgt = XG_0[nod_id-1];
                Xg(0) = Xgt(0)+dt*Vels(0); Xg(1) = Xgt(1)+dt*Vels(1); if (dim == 3){Xg(2) = Xgt(2)+dt*Vels(0);}
              }
              //Xg = getAreaMassCenterSolid(nod_id, Ar);  //this returns to the original one
              Xgt(0) = Xg(0); Xgt(1) = Xg(1); if (dim == 3){Xgt(2) = Xg(2);}
              XG[nod_id-1] = Xgt;  SV[nod_id-1] = true;
            }
            Xgt = XG[nod_id-1];
            Xg(0) = Xgt(0); Xg(1) = Xgt(1); if (dim == 3){Xg(2) = Xgt(2);}
            tmp_fs = vtheta*U1_fs + (1.-vtheta)*U0_fs;
            point->getCoord(Xp.data(),dim);  //coords node1 of current edge
            tmp_n = SolidVel(Xp,Xg,tmp_fs,dim);  //cout << tmp_n.transpose() << "  ";
        ...
        for (int k = 0; k < dof_handler_tmp[DH_t[v]].numVars(); ++k)
        {
          ...
          if ((v == 0 || v == 1) && (k == VAR_U)){
            ...
            if (nod_id){
              ...
              point->getCoord(X.data(),dim);
              Uf = SolidVel(X, XG[nod_id-1], Zf, dim);
              for (int j = 0; j < dof_handler_tmp[DH_t[v]].getVariable(k).numDofsPerVertex(); ++j)
                array[dofs_1[j]] = Uf(j);
            }
          }//end if
