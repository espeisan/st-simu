...
Vector u_exact(Vector const& X, double t, int tag)
{
  ...
  double y = X(1);
  Vector v(Vector::Zero(X.size()));
  double Um = 0.1, H = 20e-6;
  if (tag == 3){
    Um = Um*(-1-exp(t*t*t*t/1e-14)*0);
    v(0) = Um*6*(H-y)*y/(H*H);
  }
  return v;
}
...
Vector SlipVel(Vector const& X, Vector const& XG, Vector const& normal, int dim, int tag, double theta)
{
  ...
  if (dim  == 2) //this gives structure node swimm
  {
    //double theta = pi/3;
    if (false && tag == 101){
      theta = pi;
  ...
  double the = 0;
  if (false && dim == 2) //this gives total random swimm node
  {
    for (int I = 0; I < 15; I++)
    {
      if (tag == 101 + I)
      {
        the = rand() % (6);         // v1 in the range 0 to 99
